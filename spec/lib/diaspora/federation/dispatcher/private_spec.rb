  ...
  describe "#dispatch" do
    context "deliver to local user" do
      it "delivers to each user only once" do
        ...
        expect(Workers::ReceiveLocal).to receive(:perform_later).with("StatusMessage", post.id, [bob.id])
    ...
    context "deliver to remote user" do
      ...
      it "queues a private send job" do
        expect(Workers::SendPrivate).to receive(:perform_later) do |user_id, _entity_string, targets|
      ...
      it "does not queue a private send job when no remote recipients specified" do
        ...
        expect(Workers::SendPrivate).not_to receive(:perform_later)
