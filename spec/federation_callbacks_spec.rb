...
describe "diaspora federation callbacks" do
  ...
  describe ":queue_public_receive" do
    it "enqueues a ReceivePublic job" do
      data = "<diaspora/>"
      expect(Workers::ReceivePublic).to receive(:perform_later).with(data)
  ...
  describe ":queue_private_receive" do
    ...
    it "enqueues a ReceivePrivate job" do
      expect(Workers::ReceivePrivate).to receive(:perform_later).with(alice.id, data)
  ...
  describe ":receive_entity" do
    it "receives an AccountDeletion" do
      ...
      expect(Workers::ReceiveLocal).not_to receive(:perform_later)
    ...
    it "receives a entity" do
      ...
      expect(Workers::ReceiveLocal).to receive(:perform_later).with(persisted.class.to_s, persisted.id, [])
    ...
    it "receives a entity for a recipient" do
      ...
      expect(Diaspora::Federation::Receive).to receive(:perform).with(received).and_return(persisted)
      expect(Workers::ReceiveLocal).to receive(:perform_later).with(persisted.class.to_s, persisted.id, [recipient.id])
