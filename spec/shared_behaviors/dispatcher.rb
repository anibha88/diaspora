  ...
  describe "#dispatch" do
    context "deliver to local user" do
      it "queues receive local job for all local receivers" do
        ...
        expect(Workers::ReceiveLocal).to receive(:perform_later).with("StatusMessage", post.id, local_subscriber_ids)
        Diaspora::Federation::Dispatcher.build(alice, post).dispatch
      end

      it "gets the object for the receiving user" do
        expect(Workers::ReceiveLocal).to receive(:perform_later).with("RSpec::Mocks::Double", 42, [bob.id])
      ...
      it "queues receive local job for a specific subscriber" do
        expect(Workers::ReceiveLocal).to receive(:perform_later).with("StatusMessage", post.id, [eve.id])
