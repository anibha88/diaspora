...
# Creates the tables GoodJob needs to run ActiveJob jobs (DB-backed queue).
# GoodJob requires PostgreSQL (advisory locks + LISTEN/NOTIFY); this migration
# is written for Postgres and matches the GoodJob 4.x schema.
class CreateGoodJobTables < ActiveRecord::Migration[6.1]
  def change
    ...
    add_index :good_jobs, %i[locked_by_id], where: "(locked_by_id IS NOT NULL)", name: :index_good_jobs_on_locked_by_id
